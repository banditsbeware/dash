...
def load_states():
    ...
    with open("static/web-images/timeline-v3/csv/states.csv") as states:
        ...
        for state, abbr in states_map:
            if state != "State": states_names[state] = abbr
...
# Returns a string with cases & deaths for each state in `stateList`
        ...
        with open('v3/static/CDC-all-states.csv') as csvfile:
            ...
            for row in reader:
                if(row['state']==stateList[x]):
                    deaths += int(row['deathIncrease'])
                    cases  += int(row['positiveIncrease'])
            stateData += f'{stateList[x]}: {cases:,} Cases, {deaths:,} Deaths; '
...
def graph_data():
    ...
    # initialize stateData as an empty string; will contain a string of CDC information
    stateData = ''

    # initialize abbrList as an empty list; will contain the states
    # (abbreviations) which the user(s) selected to view
    abbrList = []

    # called if function called with POST API
    if request.method == "POST":

        # put the data into ipt (easier to call the information later in the code),
        ...
        for item in ipt:
            ...
            # since features in input and feature1 and feature2 at the end of the input dict,
            # if else statement ends the loop after all the states have been read
            if item != 'feature1':
