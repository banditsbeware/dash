  ...
  def main( self ) :
    ...
    if self.mapHash.get( key ) != None :
      ...
      if csvHash.get( key ) == None :
        subprocess.call(["g++", "-o", "hello", "./global_cowiz/v2/maptest/hello.cpp"])
      ...
      with open( './global_cowiz/v2/maptest/tables/html_table.txt', 'a' ) as fp :
...
#-------------------------------------------------------------
  ...
  def generateMap( self, dirName, fName ) :
    ...
    """
    path = ( './global_cowiz/v2/maptest/cases/'+ dirName + 
             '/' + fName + '.csv' )
    ...
    nDir  = ( './global_cowiz/static/maps/' + dirName + '/' )
...
#-------------------------------------------------------------
  ...
  def initCSVHash( self ) :
    ...
    open( './global_cowiz/v2/maptest/tables/csv_table.txt', 'w+' ).close()
    ...
    for path, subd, files in os.walk( './cases/' ) :
      for names in files:
        ...
        if keys[-1] == 'csv' :
          ...
          with open( './global_cowiz/v2/maptest/tables/csv_table.txt', 'a+' ) as fp :
            fp.write( key + ',' + names + '\r\n' )
          
          fp.close()

    with open( './global_cowiz/v2/maptest/tables/csv_table.txt', 'r' ) as fp :
...
#-------------------------------------------------------------
  ...
  def initMapHash( self ) : 
    ...
    with open( './global_cowiz/v2/maptest/tables/html_table.txt', 'r' ) as fp :
      lines = fp.read().replace( '\r', '' ).split( '\n' )
